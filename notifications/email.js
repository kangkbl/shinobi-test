...
module.exports = function(s,config,lang){
    ...
    try{
                ...
                s.knexQuery({
                    ...
                    r = r[0]
                        var mailOptions = {
                            ...
                            to: checkEmail(r.mail, config), // list of receivers
		...
		s.factorAuth[r.ke] &&
    	    ...
    	    const factorAuth = factorAuthObj
	    const info = factorAuthObj.info || {}
    	    const login = info.login || {}
	    
		// r = user object
            	if(r.details && r.details.factor_mail !== '0'){
                    sendMessage({
                    	from: config.mail.from,
                    	to: checkEmail(r.mail, config),
                    	subject: r.lang['2-Factor Authentication'],
                    	html: `<h2>🔐 Shinobi Login Verification</h2><p>${r.lang['Enter this code to proceed']}</p><h1 style="font-size:32px">${factorAuth.key}</h1><hr><table cellpadding="6"><tr><td><b>User</b></td><td>${r.mail}</td></tr><tr><td><b>IP Address</b></td><td>${login.ip || '-'}</td></tr><tr><td><b>Login Time</b></td><td>${login.loginTime || '-'}</td></tr><tr><td><b>Machine ID</b></td><td>${login.machineId || '-'}</td></tr><tr><td><b>Page</b></td><td>${login.pageTarget || '-'}</td></tr><tr><td><b>Host</b></td><td>${login.host || '-'}</td></tr><tr><td><b>User-Agent</b></td><td>${login.userAgent || '-'}</td></tr></table><hr><p>If this wasn't you, do not enter this code.</p>`,
                    }, (error, info) => {
                    	if (error) {
